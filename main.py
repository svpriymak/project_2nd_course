...
def extract_main_category(full_category, source_file):
    ...
    if isinstance(full_category, str) and '/' in full_category:
        ...
        # Иначе берём из имени файла (например, export_ozon_Электроника_... → Электроника)
        try:
...
data['main_category'] = data.apply(lambda row: extract_main_category(row.get('full_category'), row.get('source_file')),
                                   axis=1)

print("Объединенный DataFrame имеет размер:", data.shape)

# Топ-10 категорий по количеству товаров
top_categories = data['main_category'].value_counts().head(10)
print("\nТоп-10 категорий по количеству товаров:")
...
sns.barplot(x=top_categories.values, y=top_categories.index, hue=top_categories.index,
            palette='viridis',
            legend=False)
plt.title('Топ-10 главных категорий по количеству товаров')
...
numeric_cols = ['Balance', 'Comments', 'Rating', 'Price', 'Max price', 'Min price',
                ...
                'Days in stock', 'Days with sales', 'Average if in stock', 'full_category', 'main_category']
...
# print(
#     f"Корреляция цены и продаж: {price_sales_corr:.2f} (p={p_val_ps:.3f}); цена и наличие: {price_balance_corr:.2f} (p={p_val_pb:.3f})")
#
...
#     top_categories = category_stats[category_stats['Count'] > 10].sort_values(by='AverageRating', ascending=False).head(
#         10)
...
#
# print("\nВыводы:")
# print("- Между ценой и количеством отзывов обнаружена",
#       ("отрицательная" if price_comments_corr < 0 else "положительная"),
...
# print(
#     f"- Корреляция между числом отзывов и рейтингом: r={comments_rating_corr:.2f}. Низкое значение (близкое к 0) говорит об отсутствии сильной линейной связи.")
# print("Рекомендации:")
# print(
#     "- Фокусироваться на товарах по оптимальной цене: слишком высокая цена снижает продажи, слишком низкая – снижает маржинальность.")
# print(
#     "- Поощрять получение отзывов для более дешёвых товаров (например, акциями), так как отзывы влияют на восприятие товара.")
# print("- Обращать внимание на категории с высоким средним рейтингом; анализировать, чем они отличаются.")
# print(
#     "- Учитывать, что рейтинг сам по себе мало зависит от количества отзывов, поэтому нужно искать другие факторы повышения оценки.")
